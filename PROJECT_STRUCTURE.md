...
│   ├── 📜 baemin_module.py          # [신규] 배달의민족 쿠폰 자동 구매 (포인트 사용)
...
*   **`main_GUI.pyw`** (약 2,050줄):
    ...
    *   **`DoctorBillAutomation` 클래스**: 전체 GUI와 로직을 담당하는 핵심 클래스입니다.
        ...
        *   **설정 시스템**: `settings.json`을 통해 헤드리스 모드(`browser_headless`), 자동 작업 설정, 설정 창의 크기 등을 관리합니다.
        *   **설정 UI**: `open_settings()` 메서드로 크기 조절 및 스크롤이 가능한 설정 변경 팝업 창을 제공하며, 창 크기 기억, 저장 시 설정 창 닫기 및 브라우저 재시작을 지원합니다.
        *   **이벤트 핸들링**: 사용자 클릭 이벤트를 받아 `TaskManager`에게 작업을 요청합니다.
        *   **자동 작업**: 로그인 후 설정에 따라 출석체크, 퀴즈풀기, 세미나 확인을 자동 실행합니다.
        *   **화면 갱신**: 로그 메시지, 작업 상태, 사용자 정보, 세미나 정보를 화면에 업데이트합니다.
        *   **로깅 시스템**: `GUILogHandler`를 통해 모듈들의 파이썬 로그를 GUI 로그 창에 표시합니다.
        *   **세미나 새로고침**: 오늘의 세미나 목록을 실시간으로 확인하고, 필요 시 새로고침을 일시정지(Pause)할 수 있습니다. 새로고침 간격은 설정에서 조절 가능합니다.
        *   **자동 세미나 입장**: 입장 가능한 세미나에 시작시간 + N분 후 자동 입장합니다. 대기시간은 설정에서 조절 가능합니다.
        *   **강의 종료 감지**: 세미나 리스트에서 강의가 사라지면 종료로 감지하여 자동 설문참여를 실행합니다.
        *   **쿠폰 구매**: 닥터빌 포인트를 사용하여 배달의민족 쿠폰을 자동 구매하는 팝업 창을 제공합니다.
        *   **설문 문제 관리**: 설문 문제/정답을 관리하는 별도 팝업 창을 제공합니다.

*   **`main_task_manager.py`** (약 370줄):
    *   **중앙 관제탑** 역할을 합니다.
...
*   **`web_automation.py`** (약 200줄):
    *   **웹 브라우저 제어**의 가장 밑단에 있는 클래스입니다.
    *   `Selenium` WebDriver를 초기화하고 옵션을 설정합니다.
    *   **설정 연동**: 인스턴스 생성 시 `settings.json`의 `browser_headless` 값을 읽어 브라우저 가시성을 결정합니다.
    ...
    *   **창 관리**: `close_other_windows()`를 통해 메인 창을 제외한 모든 팝업 및 탭을 일괄 종료할 수 있습니다.
    *   **설정값**: 헤드리스 모드, 창 크기(1200x800), implicit wait(2초), page load timeout(15초) 등.

*   **`modules/base_module.py`** (약 160줄):
...
*   **`login_module.py`** (약 270줄): 닥터빌 통합회원 로그인을 처리합니다.
...
*   **`attendance_module.py`** (약 110줄): 출석체크 페이지로 이동하여 버튼을 클릭합니다.
    *   이미 출석 완료된 경우에도 정상 처리합니다.
    *   완료 후 자동으로 포인트 상태를 확인합니다.

*   **`quiz_module_new.py`** (약 720줄): 오늘의 퀴즈를 자동으로 풀어줍니다.
...
*   **`seminar_module.py`** (약 1,430줄): 라이브 세미나를 관리합니다.
...
*   **`survey_module.py`** (약 1,120줄): 설문조사를 자동으로 참여합니다.
    ...
    *   **헤드리스 모드 지원**: 정답이 없는 퀴즈 감지 시, 해당 문제 위치로 스크롤하여 **스크린샷을 자동 생성 및 팝업**합니다.
    *   **탭 정리**: 설문 참여 완료(또는 실패) 후 서브 윈도우를 모두 닫고 원래 창으로 복귀하며, 임시 스크린샷 파일을 자동 삭제합니다.

*   **`survey_problem.py`** (약 600줄): 설문 퀴즈 문제와 정답을 관리합니다.
    ...
    *   **`open_survey_problem_manager()` 함수**: GUI 관리 창 (Treeview 목록 + 입력 폼)
        *   자동 정답 등록 대기 로직 및 데이터 파일 즉시 리로드 기능 포함.

*   **`baemin_module.py`** (약 220줄): 배달의민족 쿠폰을 자동 구매합니다.
    *   닥터빌 포인트를 빌마켓 쿠폰으로 전환하는 복잡한 과정을 자동화합니다.
    *   포인트 조회 → 빌마켓 이동 → 쿠폰 페이지 접근 → 수량/연락처 입력 → 결제 및 포인트 적용의 전체 흐름을 처리합니다.
    *   마이페이지에서 사용자의 휴대폰 번호를 자동으로 추출하여 입력 편의성을 제공합니다.

*   **`points_check_module.py`** (약 200줄): 포인트 상태와 활동 내역을 확인합니다.
    *   메인 페이지에서 사용자 이름 수집
    *   포인트 페이지에서 포인트 + 출석/퀴즈 참여 상태 일괄 수집
    *   GUI 대시보드(사용자 정보 패널)를 직접 업데이트합니다.

*   **`blog_search_module.py`** (약 390줄): 네이버 블로그에서 퀴즈 정답을 검색합니다.
...
*   **`update_program.py`** (약 350줄): GitHub 기반 자동 업데이트
...
*   **`settings.json`**: 사용자 설정
    ...
    *   `auto_survey`: 강의 종료 감지 시 자동 설문참여
    *   `auto_seminar_join`: 세미나 자동 신청
    *   `auto_seminar_enter`: 세미나 자동 입장
    *   `seminar_enter_delay`: 세미나 입장 대기시간 (분)
    *   `seminar_refresh_interval`: 세미나 새로고침 간격 (초)
    *   `settings_window_width`, `settings_window_height`: 설정 창의 마지막 너비와 높이
...
7.  **자동 작업 실행**: 설정에 따라 세미나 확인 → 출석체크 → 퀴즈풀기 순서로 자동 실행.
8.  **세미나 새로고침 시작**: 설정된 간격으로 세미나 정보 갱신 + 자동 신청/입장/설문참여 처리.
